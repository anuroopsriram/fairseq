...
def get_grid(args):
    ...
    return [
        ...
        hp("--criterion", "ctc"),
        hp("--arch", "wav2vec_ctc"),
        # hp("--w2v-path", f'{args.checkpoints_dir}/checkpoint_best.pt'),
        hp("--w2v-path", f'{args.checkpoints_dir}/checkpoint_last.pt'),
        hp("--no-pretrained-weights", False, binary_flag=True),

        # hp("--wer-args", f"('{lm_bin}','{lexicon}',2,-1)"),
        hp('--normalize', [True], binary_flag=True, save_dir_key=lambda _: "norm"),
        ...
        hp("--max-update", config.LAB_DATASETS[args.dataset]["steps"], save_dir_key=lambda val: f"u{val}"),
        hp("--warmup-steps", int(config.LAB_DATASETS[args.dataset]["steps"] * .1), save_dir_key=lambda val: f"wstep{val}"),
        hp("--hold-steps", int(config.LAB_DATASETS[args.dataset]["steps"] * .0), save_dir_key=lambda val: f"hstep{val}"),
        hp("--decay-steps", int(config.LAB_DATASETS[args.dataset]["steps"] * .9), save_dir_key=lambda val: f"dstep{val}"),
        hp("--final-lr-scale", [0.05], save_dir_key=lambda val: f"lrfs{val}"),
        hp('--update-freq', [1], save_dir_key=lambda val: f"ufreq{val}"),
        hp("--max-tokens", 1280000, save_dir_key=lambda val: f"maxtok{val}"),
        hp("--freeze-finetune-updates", 5000, save_dir_key=lambda val: f"ffu{val}"),
        ...
        hp('--mask-length', 10, save_dir_key=lambda val: f"ml{val}"),
        hp('--mask-other', 0),
        hp('--mask-prob', 0.3, save_dir_key=lambda val: f'mp{val}'),
