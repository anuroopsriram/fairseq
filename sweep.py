...
def get_args():
    ...
    parser.add_argument('-t', '--num-trials', type=int, default=-1,
    ...
    parser.add_argument('--job-dir',
                        default=None,
                        help='overwrites auto created dir if set')
    ...
    parser.add_argument('--train-file', help='path to train file', default='train.py')

    parser.add_argument('--salloc', action='store_true', default=False)
...
class hyperparam(object):
    ...
    def get_save_dir_key(self):
        if self.save_dir_key is None:
            return None
        if self.binary_flag:
            return self.save_dir_key(1).replace('.', '_') if self.current_value else None
        return self.save_dir_key(self.current_value).replace('.', '_')
...
def copy_all_python_files(source, snapshot_main_dir, code_snapshot_hash):
    ...
    all_pys = glob(os.path.join(source, 'fairseq/**/*.py'), recursive=True) \
        ...
        + glob(os.path.join(source, 'fairseq/**/*.c'), recursive=True) \
        + glob(os.path.join(source, 'fairseq/**/*.cpp'), recursive=True) \
...
def launch_train(args, config):
    ...
    save_dir = os.path.join(args.checkpoints_dir, args.prefix, f'{args.prefix}.{save_dir_key}.ngpu{num_total_gpus}') if args.job_dir is None else args.job_dir
    if args.local:
        save_dir = '/tmp'
    # save_dir = os.path.join(args.checkpoints_dir, f'{args.prefix}.{save_dir_key}.ngpu16')
    ...
    train_cmd = ['python', os.path.join(destination, args.train_file)]
    ...
    if args.post_steps:
        for post_step in args.post_steps:
            if os.path.isfile(post_step):
                ...
                post_cmd = post_step
            post_cmd = post_cmd.strip().format(job_dir=save_dir, data_dir=args.data)
    ...
    if args.local:
        ...
        sbatch_cmd = [
            ...
            '--cpus-per-task', str(int(10 * args.num_gpus / ntasks_per_node)),
            '--output', train_log,
            '--error', train_stderr,
            '--open-mode', 'append',
            '--signal', 'B:USR1@180',
