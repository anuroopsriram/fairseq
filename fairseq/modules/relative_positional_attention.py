...
class RelLearnableMultiHeadAttn(RelMultiHeadAttn):
    ...
    def forward(self, w, r_emb, r_w_bias, r_bias, attn_mask=None, mems=None, need_weights=False):
        ...
        if attn_mask is not None and attn_mask.any().item():
            if attn_mask.dim() == 2:
                attn_score.masked_fill_(attn_mask.T[None, :, :, None], -float('inf'))
            elif attn_mask.dim() == 3:
        ...
        if need_weights:
            ...
            attn_weights = attn_prob.view(bsz, self.n_head, qlen, qlen)
